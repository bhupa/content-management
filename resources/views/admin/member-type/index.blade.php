        ...
        $(function(){
            ...
            $('#sortable').sortable({
                ...
                update: function(event, ui){
                    ...
                    var url = "{{ url('admin/member-type/sort') }}";
                    $.ajax({
                        type: "POST",
                        url: url,
                        datatype: "json",
            ...
            $(".defaultTable").on("click", ".change-status", function () {
                ...
                Swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('admin.member-type.change-status') }}",
                            data: {
                                'id': id,
                            },
                            dataType: 'json',
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                @can('master-policy.perform', ['content', 'add'])
                    <a href="{{ route('admin.member-type.create') }}" class="btn btn-default legitRipple pull-right">
        ...
        <div class="panel-body">
            <table class="table datatable-column-search-inputs defaultTable">
                ...
                <tbody>
                @foreach($membersType as $index=>$content)
                <tr>
                <td>{{ $index+1 }}</td>
                <td>{{ $content->name }}</td>

                <td>
                @can('master-policy.perform', ['member-type', 'changeStatus'])
                ...
                <td>
                @can('master-policy.perform', ['member-type', 'edit'])
                <a href="{{ route('admin.member-type.edit',$content->id) }}"
                title="Edit-Content"
                data-toggle="tooltip"
