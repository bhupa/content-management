        ...
        $(function(){
            ...
            $('#sortable').sortable({
                ...
                update: function(event, ui){
                    ...
                    var url = "{{ url('admin/contact/sort') }}";
                    $.ajax({
                        type: "POST",
                        url: url,
                        datatype: "json",
            ...
            $(".defaultTable").on("click", ".change-status", function () {
                ...
                Swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: "{{ url('admin/contact/change-status') }}",
                            data: {
                                'id': id,
                            },
                            dataType: 'json',
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            ...
            <div class="heading-elements">
                @can('master-policy.perform', ['contact', 'add'])
                    <a href="{{ route('admin.contact.create') }}" class="btn btn-default legitRipple pull-right">
                ...
                @foreach($contacts as $index=>$contact)
                    <tr id="item-{{ $contact->id }}">
                      ...
                      <td>{{$contact->email}}</td>
                        ...
                        <td>
                            @can('master-policy.perform', ['contact', 'edit'])
                            ...
                            @endif
                            @can('master-policy.perform', ['contact', 'delete'])
                                <a href="javascript:void(0)" id="{{  $contact->id  }}"
