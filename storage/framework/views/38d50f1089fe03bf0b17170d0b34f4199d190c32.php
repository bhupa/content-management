         ...
         $(function(){
            ...
            $('#sortable').sortable({
                ...
                update: function(event, ui){
                    ...
                    var url = "<?php echo e(url('admin/article/sort')); ?>";
                    $.ajax({
                        type: "POST",
                        url: url,
                        datatype: "json",
        ...
        $(document).ready(function () {
            $(".defaultTable").on("click", ".change-status", function () {
                ...
                Swal({
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?php echo e(route('admin.article.change-status')); ?>",
                        data: {
                            'id': id,
                        },
                        dataType: 'json',
...
<?php $__env->startSection('content'); ?>
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title"><i class="icon-grid3 position-left"></i>Article</h5>
            <div class="heading-elements">
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['article', 'add'])): ?>
                    <a href="<?php echo e(route('admin.article.create')); ?>" class="btn btn-default legitRipple pull-right">
        ...
        <div class="panel-body">
            <table class="table datatable-column-search-inputs defaultTable">
                ...
                <tr>
                    <td><?php echo e($key  + $articles->firstItem()); ?></td>
                <td><?php echo e($article->title); ?></td>
                <td><?php echo e($article->url); ?></td>
                <td><?php echo $article->short_description; ?></td>
                <td>
                    <?php if(file_exists('storage/'.$article->image) && $article->image !== ''): ?>
                        <img src="<?php echo e(asset('storage/'.$article->image)); ?>" class="displayimage" style="width:100px; height:100px; margin-bottom: 15px;" alt=""></br>
                    <?php endif; ?>
                </td>
                    <td>
                ...
                <td>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['article', 'changeStatus'])): ?>
                        <a href="javascript:void(0)"
                        class="btn btn-primary btn-icon btn-rounded legitRipple change-status"
                        id="<?php echo e($article->id); ?>">
                ...
                <td>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['article', 'edit'])): ?>
                        <a href="<?php echo e(route('admin.article.edit',$article->id)); ?>"
                        class="btn btn-success btn-icon btn-rounded legitRipple">
                        <i class=" icon-database-edit2"></i>
                        </a>
                    <?php endif; ?>
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['article', 'delete'])): ?>
                            <a href="javascript:void(0)" id="<?php echo e($article->id); ?>"
            ...
            <?php echo $articles->render('vendor.pagination.default'); ?>
