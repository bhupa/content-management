...
<link rel="stylesheet" href="<?php echo e(asset('backend/tablednd.css')); ?>">
    ...
    <script>
        ...
        var url = "<?php echo e(url('admin/download/sort')); ?>";
        $.ajax({
        type: "POST",
        url: url,
        datatype: "json",
        ...
        $.ajax({
        type: "POST",
        url: "<?php echo e(route('admin.download.change-status')); ?>",
        data: {
        'id': id,
        },
        dataType: 'json',
    ...
    <div class="panel panel-flat">
        <div class="panel-heading">
            <h5 class="panel-title"><i class="icon-grid3 position-left"></i> Publication</h5>
            <div class="heading-elements">
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['download', 'add'])): ?>
                <a href="<?php echo e(route('admin.download.create')); ?>" class="btn btn-default legitRipple pull-right">
  ...
  <?php $__currentLoopData = $downloads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$download): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr id="item-<?php echo e($download->id); ?>">
                        <td><?php echo e($key+$downloads->firstItem()); ?></td>
                        <td><?php echo e($download->title); ?></td>
                        <td><?php echo e($download->type); ?></td>
                        <td>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['download', 'changeStatus'])): ?>
                            <a href="javascript:void(0)"
                               class="btn btn-primary btn-icon btn-rounded legitRipple change-status"
                               id="<?php echo e($download->id); ?>">
                                <?php if($download->is_active == 1): ?>
                        ...
                        <td>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['download', 'edit'])): ?>
                            <a href="<?php echo e(route('admin.download.edit',$download->id)); ?>"
                               class="btn btn-success btn-icon btn-rounded legitRipple">
                                <i class=" icon-database-edit2"></i>
                            </a>
                            <?php endif; ?>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('master-policy.perform', ['download', 'delete'])): ?>
                            <a href="javascript:void(0)" id="<?php echo e($download->id); ?>"
